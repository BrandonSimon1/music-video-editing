...
def analyze_music_structure(y, sr, hop_length=512):
    ...
    try:
        ...
        tempo_value = 120.0  # Default tempo
        beats = np.array([])
    ...
    return novelty_scores, beats, tempo_value
...
def detect_interesting_moments(novelty_scores, sr, hop_length, 
    ...
    # Ensure the array is float64 and contiguous for librosa
    combined_novelty = np.ascontiguousarray(combined_novelty, dtype=np.float64)
    ...
    delta = float(threshold * 0.3)  # Minimum peak prominence - ensure it's float
    ...
    logger.info(f"    Peak detection parameters: delta={delta:.4f}, pre_max={pre_max}, wait={wait}")
    logger.info(f"    Combined novelty shape: {combined_novelty.shape}, dtype: {combined_novelty.dtype}")
    ...
    try:
        ...
        logger.info(f"    Novelty stats: min={np.min(combined_novelty):.4f}, max={np.max(combined_novelty):.4f}, mean={np.mean(combined_novelty):.4f}")
        # Fallback: simple peak detection
        peaks = []
        for i in range(pre_max, len(combined_novelty) - post_max):
            if combined_novelty[i] > threshold and combined_novelty[i] == np.max(combined_novelty[i-pre_max:i+post_max+1]):
                if len(peaks) == 0 or i - peaks[-1] >= wait:
                    peaks.append(i)
        peaks = np.array(peaks)
        logger.info(f"    Used fallback peak detection, found {len(peaks)} peaks")
